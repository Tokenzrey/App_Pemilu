...
public abstract class Polynomial {
    ...
    public Polynomial(NTRUParameterSet nTRUParameterSet) {
        this.coeffs = new short[nTRUParameterSet.m69n()];
    ...
    public void mod3PhiN() {
        int m69n = this.params.m69n();
        for (int i = 0; i < m69n; i++) {
            short[] sArr = this.coeffs;
            sArr[i] = mod3((short) (sArr[i] + (sArr[m69n - 1] * 2)));
        }
    }
    ...
    public void modQPhiN() {
        ...
        for (int i = 0; i < m69n; i++) {
            short[] sArr = this.coeffs;
            sArr[i] = (short) (sArr[i] - sArr[m69n - 1]);
        }
    }
    ...
    public void rqToS3(Polynomial polynomial) {
        int length = this.coeffs.length;
        for (int i = 0; i < length; i++) {
            this.coeffs[i] = (short) modQ(polynomial.coeffs[i] & UShort.MAX_VALUE, this.params.m68q());
            short[] sArr = this.coeffs;
    ...
    public void trinaryZqToZ3() {
        ...
        for (int i = 0; i < length; i++) {
            short[] sArr = this.coeffs;
            sArr[i] = (short) modQ(sArr[i] & UShort.MAX_VALUE, this.params.m68q());
            short[] sArr2 = this.coeffs;
            short s = sArr2[i];
    ...
    public void z3ToZq() {
        ...
        for (int i = 0; i < length; i++) {
            short[] sArr = this.coeffs;
            short s = sArr[i];
            sArr[i] = (short) (s | ((-(s >>> 1)) & (this.params.m68q() - 1)));
        }
    }
}
